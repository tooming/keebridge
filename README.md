...
- **A real secrets-management UI** in the app itself — browse/search entries, reveal fields, edit, delete. Copying a password auto-clears it from the system pasteboard after 30 seconds, unless something else has already overwritten it in the meantime; non-secret fields (username, URL, passkey metadata) keep the old no-auto-clear copy behavior.
